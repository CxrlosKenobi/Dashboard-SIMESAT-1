...
def get_mpu9250_gy_x_data(start, end):
    con = sqlite3.connect(str(DB_FILE))
    statement = f'SELECT mpu9250_gy_x FROM data WHERE timeinsec > {start} AND timeinsec <= {end};'
    df = pd.read_sql_query(statement, con)
    return df

def get_mpu9250_gy_y_data(start, end):
    con = sqlite3.connect(str(DB_FILE))
    statement = f'SELECT mpu9250_gy_y FROM data WHERE timeinsec > {start} AND timeinsec <= {end};'
    df = pd.read_sql_query(statement, con)
    return df

def get_mpu9250_gy_z_data(start, end):
    con = sqlite3.connect(str(DB_FILE))
    statement = f'SELECT mpu9250_gy_z FROM data WHERE timeinsec > {start} AND timeinsec <= {end};'
    df = pd.read_sql_query(statement, con)
    return df
